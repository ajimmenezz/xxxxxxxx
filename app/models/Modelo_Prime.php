...
class Modelo_Prime extends Modelo_Base
{
    ...
    public function getSucursalesPrime()
    {
        ...
        cs.Id,
        UPPER(cs.NombreCinemex) as Clave,
        cs.Nombre,
        regionCliente(cs.IdRegionCliente) as Region
        from cat_v3_sucursales cs
        where cs.IdCliente = 20
        and cs.Flag = 1
        order by Nombre");
        return $consulta;
    }

    public function getGeneralesInventarioSucursal(int $id)
    {
        $consulta = $this->consulta("select
        tst.Id,
        tst.Ticket,
    ...
    public function getInventarioSucursal(int $id)
    {
        $consulta = $this->consulta("select 
        tc.Id,
        UPPER(cs.NombreCinemex) as CVE_PDV,
        cs.Nombre,
        (select Nombre from cat_v3_regiones_cliente where Id = cs.IdRegionCliente) as Region,
        UPPER(linea(lineaByModelo(tc.IdModelo))) as Linea,
        UPPER(sublinea(sublineaByModelo(tc.IdModelo))) as Sublinea,
        UPPER(marca(marcaByModelo(tc.IdModelo))) as Marca,
        UPPER((select Nombre from cat_v3_modelos_equipo where Id = tc.IdModelo)) as Modelo,
        UPPER(tc.Serie) as Serie,
        estatus(tc.IdEstatus) as EstadoEquipo,
        tc.Extra as Etiqueta,
        (select Nombre from cat_v3_sistemas_operativos where Id = tc.IdSistemaOperativo) as SO,
        tc.NombreRed,
        case
            when tc.IdEstatusSoftwareRQ = 1 then 'Instalado y arranca'
            when tc.IdEstatusSoftwareRQ = 2 then 'Instalado, NO Arranca'
            when tc.IdEstatusSoftwareRQ = 3 then 'No instalado'
        end as SoftwareRQ,
        tc.MAC
        from t_servicios_ticket tst
        inner join t_censos tc on tst.Id = tc.IdServicio
        inner join cat_v3_sucursales cs on tst.IdSucursal = cs.Id
        where tst.Id = '" . $id . "'
        order by CVE_PDV, Linea, Sublinea, Marca, Modelo");
        return $consulta;
    }
