$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-metodos-procedimientos', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-metodos-procedimientos tbody').on('click', 'tr', function () {
        var datos = $('#data-table-metodos-procedimientos').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                $('#btnIniciarServicio').on('click', function () {
                    ...
                    evento.enviarEvento('Seguimiento/Servicio_Datos', data, '#modal-dialogo', function (respuesta) {
                        ...
                        recargandoTablaTesoreria(respuesta.informacion);
                    });
                });
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoMetodosProcedimientos');
            }
        }
    });
        ...
        evento.enviarEvento('Seguimiento/Servicio_Datos', data, panel, function (respuesta) {
            ...
            if (datosDelServicio.tieneSeguimiento === '0') {
                servicios.ServicioSinClasificar(
                        formulario,
                        '#listaMetodosProcedimientos',
                        '#seccionSeguimientoServicio',
                        datosTabla[0],
                        datosDelServicio,
                        'Seguimiento',
                        archivo,
                        '#panelSeguimientoMetodosProcedimientos',
                        datosTabla[1],
                        avanceServicio,
                        idSucursal,
                        datosSD
    ...
    var recargandoTablaTesoreria = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-metodos-procedimientos');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-metodos-procedimientos', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});