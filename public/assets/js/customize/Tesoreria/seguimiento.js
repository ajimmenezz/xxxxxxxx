$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-tesoreria', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-tesoreria tbody').on('click', 'tr', function () {
        var datos = $('#data-table-tesoreria').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoTesoreria');
            }
        }
    });
    ...
    var recargandoTablaTesoreria = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-Tesoreria');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-Tesoreria', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});