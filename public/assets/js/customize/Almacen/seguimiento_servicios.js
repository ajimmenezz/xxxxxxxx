$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-almacen', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-almacen tbody').on('click', 'tr', function () {
        var datos = $('#data-table-almacen').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoAlmacen');
            }
        }
    });
    ...
    var recargandoTablaGeneral = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-almacen');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-almacen', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});
