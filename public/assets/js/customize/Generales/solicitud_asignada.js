    ...
    $('#data-table-solicitudes-asignadas tbody').on('click', 'tr', function () {
        ...
        evento.enviarEvento('Solicitud/Solicitud_Datos', data, '#seccionSolicitudesAsignadas', function (respuesta) {
            if (respuesta.datosResolucionSD !== undefined) {
                if (respuesta.datosResolucionSD.operation.result.status !== 'Success') {
                    if (respuesta.datosSD.operation.result.message === 'API key received is not associated to any technician. Authentication failed.') {
                        servicios.mensajeModal('Su clave de SD no concuerda con la proporcionada en la base de datos de AdIST.', 'Advertencia', true);
                    } else {
                        servicios.mensajeModasl(respuesta.datosSD.operation.result.message, 'Advertencia', true);
                    }
                } else {
