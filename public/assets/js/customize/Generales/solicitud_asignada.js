$(function() {
  ...
  tabla.generaTablaPersonal(
    "#data-table-solicitudes-asignadas",
    null,
    null,
    true,
    true,
  ...
  $("#data-table-solicitudes-asignadas tbody").on("click", "tr", function() {
    var _fila = this;
    var datos = $("#data-table-solicitudes-asignadas")
      .DataTable()
      .row(this)
      .data();
    ...
    evento.enviarEvento(
      "Solicitud/Solicitud_Datos",
      data,
      "#seccionSolicitudesAsignadas",
      function(respuesta) {
        if (respuesta.datosResolucionSD !== undefined) {
          ...
          $("#ocultarMostrarSD").on("click", function() {
            if ($("#contenidoSD").hasClass("hidden")) {
              ...
              $("#ocultarMostrarSD span")
                .empty()
                .append("- Menos detalles");
            } else {
              $("#contenidoSD").addClass("hidden");
              $("#ocultarMostrarSD span")
                .empty()
                .append("+ Mostrar detalles");
            }
          });
          ...
          $("#selectCliente").on("change", function() {
            ...
            if (_cliente !== "") {
              evento.enviarEvento(
                "/Proyectos2/Planeacion/SucursalesByCliente",
                { id: _cliente },
                "#modal-dialogo",
                function(respuesta) {
                  $("#selectSucursal")
                  ...
                  $.each(respuesta, function(k, v) {
                    if (
                      (creator !== "" &&
                        requester !== "" &&
                        (v.NombreCinemex == creator ||
                          v.NombreCinemex == requester)) ||
                      (sucursal_interna !== "" && v.Id == sucursal_interna)
                    ) {
                      _option = v.Id;
                    }
          ...
          $("#btnConfirmarSeguimientoSolicitud").on("click", function() {
            ...
            if (cliente !== "") {
              var datosTabla = $("#data-table-servicio")
                .DataTable()
                .rows()
                .data();
              var servicios = [];
              if (datosTabla.length > 0) {
                for (var i = 0; i < datosTabla.length; i++) {
                  servicios.push({
                    servicio: datosTabla[i][4],
                    sucursal: datosTabla[i][5],
                    atiende: datosTabla[i][6],
                    descripcion: datosTabla[i][3],
                    nombreServicio: datosTabla[i][0],
                    nombreSucursal: datosTabla[i][1]
                  });
                }
                var data = {
                  solicitud: datos[0],
                  ...
                  function(respuesta) {
                    ...
                    if (typeof respuesta === "object") {
                      if (respuesta.responce === true) {
                        ...
                        evento.cargaContenidoModal(
                          '<div class="row">\n\
                                                                <div class="col-md-12 text-center">\n\
                                                                    <div class="form-group">\n\
                                                                        <p>Se genero el ticket <strong>' +
                            respuesta.ticket +
                            "</strong>  con exito y el personal encargado de brindar \n\
                ...
                evento.mostrarMensaje(
                  "#errorGenerarTicket",
                  false,
                  "Debes definir almenos un servicio de la solicitud.",
                  3000
                );
              }
            } else {
              evento.mostrarMensaje(
                "#errorGenerarTicket",
                false,
                "Debes definir el cliente para el servicio.",
                3000
              );
            }
          });
          ...
          $("#selectReasignarParsonal").on("change", function() {
            ...
            evento.enviarEvento(
              "Solicitud/BuscarAreaDepartamento",
              dataPerfil,
              "#seccionSeguimiento",
              function(respuesta) {
                if (respuesta) {
          ...
          $("#btnConfirmarReasignar").on("click", function() {
            if (evento.validarFormulario("#formReasignarSolicitud")) {
              var data = {
                solicitud: datos[0],
                operacion: "5",
                departamento: $("#selectReasignarDepartamento").val(),
                descripcion: $("#textareaDescripcionReasignacion").val()
              };
              evento.enviarEvento(
                ...
                function(respuesta) {
                  ...
                  if (typeof respuesta === "object") {
                    ...
                    if (respuesta.solicitudes.solicitudes.length > 0) {
                      $.each(respuesta.solicitudes.solicitudes, function(
                        indice,
                        item
                      ) {
                        fila = [];
                        ...
                        if (respuesta.solicitudes.SolicitudesSD.length > 0) {
                          $.each(respuesta.solicitudes.SolicitudesSD, function(
                            key,
                            value
                          ) {
          ...
          $("#btnRechazarSolicitud").on("click", function() {
            if (datos[2] === "SERVICE DESK") {
              ...
              evento.enviarEvento(
                "Solicitud/Datos_SistemaExterno",
                data,
                "#modal-dialogo",
                function(respuesta) {
                  var data = [];
                  $.each(respuesta.tecnicosSD.operation.details, function(
                    key,
                    value
                  ) {
                    data.push({
                      id: value.TECHNICIANID,
                      text: value.TECHNICIANNAME
                    });
                  });
          ...
          $("#btnRechazarSolicitudSD").on("click", function() {
            evento.enviarEvento(
              "Solicitud/FormularioRechazarSolicitud",
              {},
              "#modal-dialogo",
              function(respuesta) {
                ...
                $("#btnRechazarSD").on("click", function() {
                  ...
                  if (descripcion !== "") {
                    ...
                    evento.mostrarMensaje(
                      "#errorRechazar",
                      false,
                      "Debes escribir la descripción del rechazo.",
                      3000
                    );
                  }
                });
            ...
            if (evento.validarFormulario("#formRechazarSolicitud")) {
              if (datos[2] === "SERVICE DESK") {
                var data = {
                  solicitud: datos[0],
                  operacion: "4",
                  tecnicoSD: $("#selectTecnicosSD").val(),
                  descripcion: $("#textareaDescripcionRechazada").val()
                };
                var mensajeExito = "Se a reasignado el folio con exito.";
                var mensajeError =
                  "No se pudo reasignar el folio Service Desk por favor de volver a intentarlo.";
              } else {
                var data = {
                  solicitud: datos[0],
                  operacion: "4",
                ...
                var mensajeExito =
                  "Se a rechazado con exito la solicitud. <br>Se le notifico al solicitante del por que no sea brindado el seguimiento.";
                var mensajeError =
                  "No se pudo generar el rechazo de la solicitud por favor de volver a intentarlo.";
              }
              evento.enviarEvento(
                "Solicitud/Solicitud_Actualizar",
                data,
          ...
          if (respuesta.autorizacionAtenderServicio) {
            ...
            $("#btnAtenderSolicitud").on("click", function() {
              ...
              evento.enviarEvento(
                "Solicitud/Atencion_Solcitud",
                data,
                "#modal-dialogo",
                function(respuesta) {
                  if (respuesta) {
                    evento.mensajeConfirmacion(
                      "Solicitud atendida",
                      "Correcto"
                    );
                  } else {
                    evento.mensajeConfirmacion(
                      "Lo sentimos se produjo un error favor de contactar al área correspondiente",
                      "Error"
                    );
                  }
                }
  ...
  var eventosSolicitudesAsiganadas = function() {
    ...
    if (typeof respuesta.datosSD === "object") {
      ...
      typeof (respuesta.datos.detalles[0]) != "undefined" &&
            ...
            function(respuesta) {
              ...
              if (typeof respuesta === "object") {
                if (respuesta.responce === true) {
                  ...
                  if (respuesta.folios != null && respuesta.folios != 0) {
                    htmlfolio =
                      " y el folio <strong>" + respuesta.folios + "</strong>";
                  }
                  let html =
                    '<div class="row">\n\
                                                <div class="col-md-12 text-center">\n\
