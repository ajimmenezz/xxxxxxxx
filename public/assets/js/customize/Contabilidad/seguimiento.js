$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-contabilidad', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-contabilidad tbody').on('click', 'tr', function () {
        var datos = $('#data-table-contabilidad').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                $('#btnIniciarServicio').on('click', function () {
                    ...
                    evento.enviarEvento('Seguimiento/Servicio_Datos', data, '#modal-dialogo', function (respuesta) {
                        ...
                        recargandoTablaContabilidad(respuesta.informacion);
                    });
                });
    ...
    var recargandoTablaContabilidad = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-contabilidad');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-contabilidad', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
