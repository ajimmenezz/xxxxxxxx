$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-laboratorio', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-laboratorio tbody').on('click', 'tr', function () {
        var datos = $('#data-table-laboratorio').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                $('#btnIniciarServicio').on('click', function () {
                    ...
                    evento.enviarEvento('Seguimiento/Servicio_Datos', data, '#modal-dialogo', function (respuesta) {
                        ...
                        recargandoTablaLoboratorio(respuesta.informacion);
                    });
                });
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoLaboratorio');
            }
        }
    });
    ...
    var recargandoTablaLoboratorio = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-laboratorio');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-laboratorio', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
