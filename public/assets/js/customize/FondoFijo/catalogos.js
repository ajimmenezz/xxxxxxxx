$(function () {
    ...
    $('#table-montos-usuario tbody').on('click', 'tr', function () {
        let _this = this;
        var datosTabla = $('#table-montos-usuario').DataTable().row(_this).data();
        var datos = {
            'id': datosTabla[0],
            'nombre': datosTabla[1]
        };

        evento.enviarEvento('Catalogos/FormularioEditarMontosUsuario', datos, '#panel-catalogos', function (respuesta) {
            if (respuesta.code == 200) {
                $("#formularioEditarMontos").empty().append(respuesta.formulario);
                evento.cambiarDiv("#listaUsuariosFondoFijo", "#formularioEditarMontos");
                initGuardarMontos();
            } else {
                evento.mostrarMensaje("#errorMessage", false, respuesta.error, 4000);
            }
