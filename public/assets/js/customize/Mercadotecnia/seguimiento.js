$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-mercadotecnia', null, null, true, true, [[0, 'desc']]);
    ...
    //Evento que carga la seccion de seguimiento de un servicio de Mercadotecnia
    $('#data-table-mercadotecnia tbody').on('click', 'tr', function () {
        var datos = $('#data-table-mercadotecnia').DataTable().row(this).data();
        if (datos !== undefined) {
    ...
    var recargandoTablaGeneral = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-mercadotecnia');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-mercadotecnia', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});
