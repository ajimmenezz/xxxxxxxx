$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-administrador', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-administrador tbody').on('click', 'tr', function () {
        var datos = $('#data-table-administrador').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                $('#btnIniciarServicio').on('click', function () {
                    ...
                    evento.enviarEvento('Seguimiento/Servicio_Datos', data, '#modal-dialogo', function (respuesta) {
                        ...
                        recargandoTablaAdministrador(respuesta.informacion);
                    });
                });
        ...
        evento.enviarEvento('Seguimiento/Servicio_Datos', data, panel, function (respuesta) {
            ...
            if (datosDelServicio.tieneSeguimiento === '0') {
                servicios.ServicioSinClasificar(
                        ...
                        'Seguimiento',
                        archivo,
                        '#panelSeguimientoAdministracion',
                        datosTabla[1],
                        avanceServicio,
                        idSucursal,
                        datosSD
    ...
    var recargandoTablaAdministrador = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-administrador');
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-administrador', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});