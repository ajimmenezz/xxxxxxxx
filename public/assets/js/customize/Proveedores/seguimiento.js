$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-proveedores', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-proveedores tbody').on('click', 'tr', function () {
        ...
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoProveedores');
            }
        }
    });
    ...
    var recargandoTablaGeneral = function (informacionServicio) {
        ...
        $.each(informacionServicio.serviciosAsignados, function (key, item) {
            tabla.agregarFila('#data-table-proveedores', [item.Id, item.Ticket, item.Servicio, item.FechaCreacion, item.Descripcion, item.NombreEstatus, item.IdEstatus, item.Folio]);
        });
    };
});
