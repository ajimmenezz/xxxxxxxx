$(function () {
    ...
    tabla.generaTablaPersonal('#data-table-facturacion-cobranza', null, null, true, true, [[0, 'desc']]);
    ...
    $('#data-table-facturacion-cobranza tbody').on('click', 'tr', function () {
        var datos = $('#data-table-facturacion-cobranza').DataTable().row(this).data();
        if (datos !== undefined) {
            ...
            if (operacion === '1') {
                ...
                cargarFormularioSeguimiento(data, datos, '#panelSeguimientoFacturacionCobranza');
            }
        }
    });
        ...
        evento.enviarEvento('Seguimiento/Servicio_Datos', data, panel, function (respuesta) {
            ...
            if (datosDelServicio.tieneSeguimiento === '0') {
                servicios.ServicioSinClasificar(
                        formulario,
                        '#listaFacturacionCobranza',
                        '#seccionSeguimientoServicio',
                        datosTabla[0],
                        datosDelServicio,
                        'Seguimiento',
                        archivo,
                        '#panelSeguimientoFacturacionCobranza',
                        datosTabla[1],
                        avanceServicio,
                        idSucursal,
                        datosSD
    ...
    var recargandoTablaTesoreria = function (informacionServicio) {
        tabla.limpiarTabla('#data-table-facturacion-cobranza');
